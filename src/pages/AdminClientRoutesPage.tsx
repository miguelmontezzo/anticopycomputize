...
import { Plus, ExternalLink, Copy, Check, FolderKanban } from 'lucide-react';
            ...
            <FadeIn delay={0.2}>
                <div className="grid grid-cols-1 gap-4">
                    {loading ? <div className="text-muted">Carregando...</div> : items.map(item => {
                        ...
                        const p4 = `${window.location.origin}/calendario/${item.slug}`;
                        const p5 = `${window.location.origin}/admin/calendario/${item.slug}`;
                        return (
                            <div key={item.id} className="bg-black/30 border border-white/10 rounded-xl p-4">
                                ...
                                <div className="mt-3 grid grid-cols-1 md:grid-cols-5 gap-2 text-xs">
                                    {[{ label: 'Apresentação', url: p1 }, { label: 'Análise', url: p2 }, { label: 'Form EMP', url: p3 }, { label: 'Calendário público', url: p4 }, { label: 'Calendário admin', url: p5 }].map(row => (
